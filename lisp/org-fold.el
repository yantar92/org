...
Return nil when no fold is present at point of POM."
  ...
  (org-with-point-at (or pom (point))
	...
	(let ((beg (point))
	      (end (point)))
          (while (org-fold-get-folding-spec nil (max (point-min) (1- beg)))
            (setq beg (or (car (org-find-text-property-region beg 'invisible))
			  beg)))
          (while (org-fold-get-folding-spec nil end)
            (setq end (or (cdr (org-find-text-property-region end 'invisible))
			  end)))
          (unless (eq beg end)
            (cons beg end)))))))
