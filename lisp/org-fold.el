...
Return nil when no fold is present at point of POM."
  ...
  (org-with-point-at (or pom (point))
	...
	(org-find-text-property-region (point) (org-fold--property-symbol-get-create spec nil t))
      (when (org-fold-get-folding-spec)
	(let ((beg (point))
	      (end (point)))
          (while (and (> (point) (point-min))
		      (org-fold-get-folding-spec nil (max (point-min) (1- beg))))
	    (setq beg (or (car (org-find-text-property-region beg 'invisible))
			  beg)))
          (while (and (< (point) (point-max))
		      (org-fold-get-folding-spec nil (min (1+ end) (point-max))))
	    (setq end (or (cdr (org-find-text-property-region (min (1+ end) (point-max)) 'invisible))
			  end)))
          (unless (eq beg end)
            (cons beg end)))))))
