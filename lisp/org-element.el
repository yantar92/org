...
(eval-and-compile
  (defconst org-element-TBLFM-keyword "TBLFM"
    "Table formula keyword string.")

  (defvaralias 'org-TBLFM-regexp 'org-element-TBLFM-regexp)
  (defconst org-element-TBLFM-regexp
    (rx-to-string `(seq bol (0+ (any " \t")) "#+" ,org-element-TBLFM-keyword ": "))
    "Detect a #+TBLFM line."))

(defvaralias 'org-table-hline-regexp 'org-element-table-hline-regexp)
(defconst org-element-table-hline-regexp "^[ \t]*|-"
  ...
  (save-excursion
    (let* ((case-fold-search t)
	   ...
	   (tblfm (let (acc)
		    (while (looking-at
                            (eval-and-compile
                              (concat org-element-TBLFM-regexp
                                      " *\\(.*\\)[ \t]*$")))
...
CONTENTS is a string, if table's type is `org', or nil."
  (if (eq (org-element-property :type table) 'table.el)
    ...
    (concat (with-temp-buffer (insert contents)
			      (org-table-align)
			      (buffer-string))
	    (mapconcat (lambda (fm) (concat "#+" org-element-TBLFM-keyword ": " fm))
...
`:post-affiliated' properties."
  (save-excursion
    (let* ((type (if (looking-at-p org-element-table-hline-regexp) 'rule 'standard))
	   (begin (point))
