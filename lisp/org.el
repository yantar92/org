...
\\{org-mode-map}"
  ...
  (org-fold-initialize)
  (when org-link-descriptive (org-fold-add-folding-spec 'org-link nil 'hide-completely 'no-isearch-open 'append))
		...
		(remove-text-properties (match-beginning 2) (match-end 2)
	      ...
	      (when (and org-hide-emphasis-markers
			 (not (org-at-comment-p)))
                (org-fold-region (match-end 4) (match-beginning 5) t 'org-link)
                (org-fold-region (match-beginning 3) (match-end 3) t 'org-link))
	      (throw :exit t))))))))
	...
	(when (and (memq style org-highlight-links)
	  ...
	  (let* ((link-object (save-excursion
		...
		(add-text-properties start end properties)
                ...
                (org-fold-region visible-end end t spec)
