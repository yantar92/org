...
(defvar-local org-src--allow-write-back t
  "When non-nil, write back to original Org buffer before exiting.
When set to a function, call that function in a temporary buffer
populated with the contents about to be written into the original src
block.  The function may edit the temporary buffer to produce the text
to be written back.")
(put 'org-src--allow-write-back 'permanent-local t)

(defvar-local org-src--babel-info nil
  "Src block info for the original src block in Org buffer.
The value is returned by `org-babel-get-src-block-info'.")
(put 'org-src--babel-info 'permanent-local t)

(defvar-local org-src--beg-marker nil
  "Beginning of the original src block text area.")
(put 'org-src--beg-marker 'permanent-local t)

(defvar-local org-src--block-indentation nil
  "Original src block indentation.")
(put 'org-src--block-indentation 'permanent-local t)

(defvar-local org-src--content-indentation nil
  "Indentation to be used for src block contents on writeback.")
(put 'org-src--content-indentation 'permanent-local t)

(defvar-local org-src--preserve-indentation nil
  "When non-nil, do not change indentation when writing back.
Setting this variable to non-nil, makes org-src ignore
`org-src--content-indentation'.")
(put 'org-src--preserve-indentation 'permanent-local t)

(defvar-local org-src--end-marker nil
  "End of the original src block text area.")
(put 'org-src--end-marker 'permanent-local t)

(defvar-local org-src--from-org-mode nil
  "When non-nil, the original buffer's mode derives from `org-mode'.")
(put 'org-src--from-org-mode 'permanent-local t)

(defvar-local org-src--overlay nil
  "Overlay displayed over original src block area.")
(put 'org-src--overlay 'permanent-local t)

(defvar-local org-src--remote nil
  "When non-nil. do not preserve point when moving to the source.
See REMOTE argument in `org-src--edit-element'.")
(put 'org-src--remote 'permanent-local t)

(defvar-local org-src--saved-temp-window-config nil
  "Window configuration before display org-src edit window.")
(put 'org-src--saved-temp-window-config 'permanent-local t)

(defvar-local org-src--source-type nil
  "Type of Org element being edited, as a symbol.")
