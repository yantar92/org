...
The function `org-babel-schedule:LANG' must be defined like this:
    ...
    #+begin_src elisp
    (defun org-babel-schedule:LANG (body params sentinel)
      ...
      )
    #+end_src

that is, compared to `org-babel-execute:LANG', it takes one extra
parameter: the SENTINEL function.
...
must setup calls to SENTINEL to communicate to org the progress and
the outcome. It must return immediately a TASK-CONTROL.
...
this PENREG; `org-babel-schedule:LANG' must return a TASK-CONTROL.
Org then connects that TASK-CONTROL to the pending region PENREG.
...
return immediately a TASK-CONTROL.
