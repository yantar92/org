...
Check [[*Interaction between built-in and manually installed Org]] for
more details.
#+end_quote
...
** Interaction between built-in and manually installed Org
:PROPERTIES:
:ALT_TITLE: Mixed Org mode installation
:DESCRIPTION: Dealing with multiple mixed Org versions
:END:
#+cindex: mixed installation, installation error, installation failure, straight.el

When Org mode is installed using ELPA package archive or from Git
sources, multiple versions of Org mode will be present available to
Emacs.  Then, depending on the specifics of Emacs configuration,
package manager being used, and Org installation procedure, Emacs may
sometimes load some Org libraries from built-in Org and some Org
libraries from the manually installed Org version.  Such scenario is
called "mixed installation".

Mixed installation may lead to random errors caused by incompatible
library versions.

Org will raise a warning if mixed installation is detected.
Alternatively, running =M-x org-version= may also detect and indicate
mixed installation (a message containing "mixed installation!" will be
printed in the echo area).

Below, we list some known scenarios when mixed installation may happen
and suggestions how to deal with the problem.

- Some compiled Org libraries are not re-compiled after Org update.
  ...
  In Emacs 28 and earlier, it may also be necessary to re-install Org
  using Emacs with minimal configuration (=emacs -Q=):

  1. Uninstall Org
  2. Make sure that Org ELPA directory is deleted (defaults to
     =.emacs.d/elpa=)
  3. Start Emacs with -Q option
  4. Install Org using M-x list-packages
  5. Quit Emacs and restart it

- Emacs is loaded using literate Org config and more recent Org mode
  version is loaded inside the file loaded by ~org-babel-load-file~.
  ~org-babel-load-file~ triggers the built-in Org version clashing
  the newer Org mode version attempt to be loaded later.

  It is recommended to move the Org loading code before the
  ~org-babel-load-file~ call.

- New Org mode version is loaded manually by setting ~load-path~, but
  some other package depending on Org is loaded before the
  ~load-path~ is configured.  This "other package" is triggering
  built-in Org mode version, again causing the version mismatch.

  It is recommended to set ~load-path~ as early in the config as
  possible.

- New Org mode version is loaded using straight.el package manager
  and other package depending on Org mode is loaded before straight
  triggers loading of the newer Org version.
